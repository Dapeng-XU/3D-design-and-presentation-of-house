...
//加载JSON文件
function loadJSON(){
    $.get("json/modelRoom.json",function(data,status){
...
function getInfo()
{
    ...
    $("#next").click(function(){
        //进入3D界面，整理坐标
        var nextName = $("#name").val();
        var nextData = new Array();
        var nextDoor = new Array();
        var nextWind = new Array();
        var scale = $("#scale").val();//默认为1
        if($("#scale").val()==''){
            scale = 1;
        }
        var next_room_h = $("#room_h").val();
        var next_door_h = $("#door_h").val();
        var next_window_l = $("#window_l").val();
        var next_window_h = $("#window_h").val();
        //默认高度参数
        ...
        next_var.name = nextName;
        next_var.data = nextData;//墙的坐标，直线端点
        next_var.wind = nextWind;//窗户坐标，直线端点
        next_var.door = nextDoor;//门的坐标，直线端点
        next_var.room_h = next_room_h*GRID/scale;//房间高度，数值
        next_var.door_h = next_door_h*GRID/scale;//门的高度，数值
        next_var.window_l = next_window_l*GRID/scale;//窗户下端高度，数值
        next_var.window_h = next_window_h*GRID/scale;//窗户上端高度，数值
        next_var.scale = scale;

        // alert('nextName: '+nextName+'\nnext_room_h: '+next_room_h+'\nnext_door_h: '+next_door_h+
        //     '\nnext_window_l: '+next_window_l+'\nnext_window_h: '+next_window_h+'\nnextDoor: '+
        //     nextDoor+'\nnextWind: '+nextWind+'\nnextData: '+nextData);
        ...
        //     "room_h:"+next_room_h+
        //     "door_h:"+next_door_h+
        //     "window_l:"+next_window_l+
        //     "window_h:"+next_window_h+
        //     "scale:"+scale+
        //     '}';
